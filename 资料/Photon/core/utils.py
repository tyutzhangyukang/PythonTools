...
def extract_headers(headers):
    ...
    sorted_headers = {} # 先定义一个空的字典
    # findall(pattern, string)，返回 headers 中所有与前面的正则表达式相匹配的全部字符串，返回形式为列表，类似于：[('Accept-Language', 'zh-cn'), ('Accept-Encoding', 'gzip, deflate')]
    # r 表示字符串为非转义的原始字符串，其中的字符串不进行转义，(.*)匹配分组，.* 表示匹配除换行符之外的所有字符，\s 匹配任意空白字符，以冒号作为分割
    ...
    for match in matches:
        ...
        try:
            ...
            pass
    return sorted_headers # 返回排序后的 headers
